...
//checking if request cotains multipart data
            ...
            if (isMultipart) {
                ...
                while (itr.hasNext())//checking if there is any value in the position like rs.next()
                {
                    ...
                    } else//when it is an image or multipart
                    {
                        ...
                        if (field_name.equals("filephoto"))//write the name of image here it is img_product
                        {
                            ...
                            photo = "PropertyPhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                ...
                if (value[7].equals(value[8])) {//Strinh str1="insert into tbl_rent ()";//inserting file
                    String InsQry = "INSERT INTO tbl_rent(rent_name ,rent_image, rent_amount,rent_details,renttype_id,place_id,owner_id)values('" + value[0] + "','"+photo+"','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + value[7] + "','"+session.getAttribute("oid")+"')";
