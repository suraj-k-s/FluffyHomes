<%-- 
    Document   : OwnerUploadAction
    Created on : 4 Mar, 2024, 12:39:37 PM
    Author     : rinub
--%>
...
//checking if request cotains multipart data
                ...
                while (itr.hasNext())//checking if there is any value in the position like rs.next()
                {
                    ...
                    } else//when it is an image or multipart
                    {
                        ...
                        if (field_name.equals("file_photo"))//write the name of image here it is img_product
                        {
                            ...
                            photo = "ownerPhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                     ...
                     if (field_name.equals("file_proof"))//write the name of image here it is img_product
                        {
                            ...
                            proof = "ownerproof_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
                            savedFile = new File(file_path + photo);//fn contains name of file
                            try {
                                //writing the file object
                    ...
                    String InsQry = "INSERT INTO tbl_owner( owner_name, owner_contact,owner_email,owner_address,owner_zipcode,owner_gender,owner_photo,owner_proof,owner_doj,place_id,owner_password)"
                            + "values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[3] + "','" + value[4] + "','" + value[5] + "','" + photo  + "','"+proof+"',curdate(),'" + value[9] + "','"+ value[10]+"')";
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Guest/Owner.jsp'
